...
graph requires 2180 amplitudes—computationally prohibitive. 

To address this, we implement a quantum preconditioning pipeline based on Dupont et al. (2025), where QAOA on small subgraphs extracts two-point correlations $⟨Z_iZ_j⟩$ used to reweight the global graph before classical polish. 

Two subgraph selection strategies are compared: 
- community detection (covers
63.7% of edges)
- light-cone decomposition (covers 100% of edges).

Light-cone preconditioning achieves a **+1.23% improvement** over the classical baseline for Problem B
